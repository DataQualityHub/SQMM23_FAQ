...
Output managers of regular outputs are responsible for completing an SQMM for their outputs. However, as SQMM questions cover the end-to-end production process, we encourage managers to involve and consult their teams and colleagues from other teams involved. A full list of the expected SQMM returns is available below.
...
```{r read titanic, include=FALSE}
commissionning_data <- readxl::read_excel(".\\input\\DDs_SQMM_2023_population[phase2].xlsx", sheet = 2, skip = 3) %>%
  janitor::clean_names() %>%
  filter(commissioned_date !="NA") %>% 
  select(division_2023,
         sqmm2023_expected_return_name,
         sqmm23_id,
         email_of_output_manager,
         sqmm_2022_return_if_exists) %>% 
    distinct(sqmm2023_expected_return_name,
           sqmm23_id,
           email_of_output_manager, .keep_all = TRUE) %>% 
  rename(Division = division_2023,
         Output = sqmm2023_expected_return_name,
         'Output manager(s)' = email_of_output_manager,
         'Output code' = sqmm23_id,
         'sqmm2022 return' = sqmm_2022_return_if_exists) %>% 
  arrange(Division,
          Output,
          'Output manager(s)')
```
...
```{r kable_table, echo = FALSE, results = TRUE}

commissionning_data %>% 
  kable() %>% 
  kable_styling(bootstrap_options = c("striped", "condensed")) %>% 
  kableExtra::scroll_box(width = "120", height = "1000px")
```
<br>
<br>
...
You can complete the survey [here](https://www.smartsurvey.co.uk/s/SQMM2023/). Please use the code for your output that we sent you in our email. If you cannot find this, please look for your output in Question 2.1 of this document or contact [DQHub](mailto:DQHub@ons.gov.uk). Please remember to save your survey. This will trigger an email to you with a new link that will have your saved responses. We can provide you with your personalized link if you cannot find it and have saved your responses.
<br>
<br>
...
Unfortunately, there is limited functionality to implement this in the platform we use to run the SQMM, and some questions have changed. We can provide you with a PDF copy with your responses from last year, and you can copy what is relevant into your questionnaire. Please contact [DQHub](mailto:DQHub@ons.gov.uk) to request a copy of your responses from last year.  
<br>
